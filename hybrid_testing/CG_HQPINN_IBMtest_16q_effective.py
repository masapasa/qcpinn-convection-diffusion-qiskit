#!/usr/bin/env python3
"""
16-qubit hybrid quantum PINN for axisymmetric Czochralski melt flow.

Design goals
------------
1. Pretrain locally on Aer for 2000 epochs with an *analytic* simulator mode
   so the PINN residual is still tractable.
2. Fine-tune on IBM hardware for 100 epochs with 4096 shots.
3. Make the IBM stage realistic by freezing the expensive upstream blocks and
   training only a small classical calibration head by default.

Why the staged setup looks like this
------------------------------------
A full PINN loss on real hardware is usually too expensive because the physics
loss needs first and second spatial derivatives, while shot-based hardware
training prefers gradient transforms such as parameter-shift. The hardware
stage below therefore defaults to supervised calibration only.

Example
-------
# 1) Local Aer pretraining
python CG_HQPINN_IBMtest_16q_effective.py \
    --phase aer \
    --data cz_melt_raw.txt \
    --epochs 2000 \
    --save hybrid_qpinn_16q_aer.pt

# 2) IBM fine-tuning (uses 4096 shots by default)
python CG_HQPINN_IBMtest_16q_effective.py \
    --phase ibm \
    --data cz_melt_raw.txt \
    --load hybrid_qpinn_16q_aer.pt \
    --epochs 100 \
    --save hybrid_qpinn_16q_ibm.pt \
    --backend least_busy
"""
...
def run_ibm_stage(args: argparse.Namespace, device: torch.device) -> None:
    ...
    q_layer = make_quantum_layer(qdev, args.n_qubits, args.n_layers, diff_method="parameter-shift")
    model = Hybrid16QPINN(q_layer=q_layer, n_qubits=args.n_qubits).to(device)
    ...
    loader = make_dataloader(x_calib, y_calib, batch_size=args.batch_size, shuffle=True)
    ...
    print("=" * 80)
    print("IBM HARDWARE FINE-TUNING")
    print(f"backend: {backend.name}")
    print(f"shots: {args.shots}")
    print(f"calibration subset size: {x_calib.shape[0]}")
    print(f"trainable params: {count_trainable_params(model):,}")
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if epoch == 1 or epoch % args.log_every == 0 or epoch == args.epochs:
            print(
                ...
                f"data={epoch_total / max(n_batches, 1):.4e}"
            )

    save_checkpoint(args.save, model, artifacts.stats, args)
    print(f"Saved IBM checkpoint to {args.save}")
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument("--phase", choices=["aer", "ibm"], required=True)
